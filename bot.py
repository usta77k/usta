...
# الكاذبة، أو اخفضه لزيادة الحساسية. القيمة النموذجية الموصى بها: 0.7
NSFW_CONFIDENCE_THRESHOLD = 0.70
...
logger.info("جاري تحميل نموذج كشف المحتوى الإباحي (opennsfw2) ...")
# تحميل النموذج مسبقًا مرة واحدة عند بدء التشغيل (بدلاً من تحميله ضمنيًا
# عند أول استخدام) - الأوزان مرفقة داخل الحزمة نفسها، لا تنزيل شبكي هنا.
_nsfw_model = n2.make_open_nsfw_model()
...
def _is_frame_explicit(frame_bgr: np.ndarray, tmp_dir: str) -> bool:
    """يحفظ الإطار مؤقتًا كصورة ويشغّل عليه opennsfw2، يعيد True إن كان إباحيًا."""
    tmp_path = os.path.join(tmp_dir, "frame.jpg")
    cv2.imwrite(tmp_path, frame_bgr)
    try:
        # predict_image تعيد رقمًا بين 0.0 و 1.0: احتمالية أن تكون الصورة
        # إباحية (NSFW). لا حاجة لتصنيفات/فئات كما في NudeNet.
        image = n2.preprocess_image(tmp_path, n2.Preprocessing.YAHOO)
        nsfw_probability = float(_nsfw_model.predict(np.expand_dims(image, 0), verbose=0)[0][1])
    except Exception:  # noqa: BLE001
    ...
    return nsfw_probability >= NSFW_CONFIDENCE_THRESHOLD
